...
def crawl_board_filter_poster():
    print("=" * 60)
    print("   강원대 게시판 크롤러 (특정 작성자 제외 기능 추가)")
    print("=" * 60)
...
def process_detail_page(url, ntt_id, headers, save_folder):
    ...
    try:
        ...
        # [추가된 부분] 특정 작성자 제외 로직
        # 정확히 "이정학 (이정학)"인 경우 저장하지 않고 건너뜁니다.
        # 공백이나 괄호 등을 유연하게 처리하고 싶다면 조건을 '이정학' in poster 등으로 변경 가능
        if poster == "이정학 (이정학)":
            print(f"     [제외됨] 작성자가 '{poster}'입니다.")
            return
        ...
        # 4. Timestamp
        timestamp_xpath = '/html/body/div[1]/div[2]/div[2]/main/article/div/div/div[1]/div/span[1]/strong'
